...
###### [5.2.1.1. Easily accessing credential data](https://www.w3.org/TR/webauthn-3/#sctn-public-key-easy)

> Every user of the [[[Create]](origin, options, sameOriginWithAncestors)](https://www.w3.org/TR/webauthn-3/#dom-publickeycredential-create-slot) method will need to parse and store the returned [credential public key](https://www.w3.org/TR/webauthn-3/#credential-public-key) in order to verify future [authentication assertions](https://www.w3.org/TR/webauthn-3/#authentication-assertion). However, the [credential public key](https://www.w3.org/TR/webauthn-3/#credential-public-key) is in COSE format [[RFC9052]](https://www.w3.org/TR/webauthn-3/#biblio-rfc9052), inside the credentialPublicKey member of the attestedCredentialData, inside the authenticator data, inside the attestation object conveyed by AuthenticatorAttestationResponse.attestationObject. Relying Parties wishing to use attestation are obliged to do the work of parsing the attestationObject and obtaining the credential public key because that public key copy is the one the authenticator signed. However, many valid WebAuthn use cases do not require attestation. For those uses, user agents can do the work of parsing, expose the authenticator data directly, and translate the credential public key into a more convenient format.
...
したがって、getPublicKey() 操作は、クレデンシャルの公開鍵を SubjectPublicKeyInfo として返します。この ArrayBuffer は、たとえば、Java の java.security.spec.X509EncodedKeySpec、.NET の System.Security.Cryptography.ECDsa.ImportSubjectPublicKeyInfo、または Go の crypto/x509.ParsePKIXPublicKey に渡すことができます。
...
新しいクレデンシャル・レコードは、以下のオプション・コンテンツも含んでもよい（MAY）：
...
#### [7.2. Verifying an Authentication Assertion](https://www.w3.org/TR/webauthn-3/#sctn-verifying-assertion)

> In order to perform an authentication ceremony, the Relying Party MUST proceed as follows:

認証セレモニーを実行するために、信頼当事者は以下のように進めなければならない[MUST]：
...
1.optionsを、セレモニーに対する依拠当事者のニーズに合わせて構成された新しいPublicKeyCredentialRequestOptions構造体とする。
...
2.navigator.credentials.get()を呼び出し、publicKeyオプションとしてオプションを渡します。成功裏に解決された約束の結果をcredentialとする。プロミスが拒否された場合、ユーザーから見えるエラーでセレモニーを中断するか、拒否されたプロミスで利用可能なコンテキストから判断できるように、ユーザーエクスペリエンスをガイドする。さまざまなエラーコンテキストとそれに至る状況についての情報は、セクション6.3.3 authenticatorGetAssertionオペレーションを参照のこと。

## [Concise Data Definition Language (CDDL): A Notational Convention to Express Concise Binary Object Representation (CBOR) and JSON Data Structures](https://datatracker.ietf.org/doc/html/rfc8610)
...
### [7. Key Objects](https://www.rfc-editor.org/rfc/rfc9052.html#name-key-objects)
...
#### [7.1. COSE Key Common Parameters](https://www.rfc-editor.org/rfc/rfc9052.html#name-cose-key-common-parameters)
...
本文書は、COSE Key オブジェクトの一連の共通パラメータを定義する。表 4 に、このセクションで定義されているパラメータの概要を示す。また、特定の鍵タイプ用に定義されたパラメータもある。鍵タイプ固有のパラメータは[RFC9053]に記載されている。

## [RFC 9053 CBOR Object Signing and Encryption (COSE): Initial Algorithms](https://www.rfc-editor.org/rfc/rfc9053.html)
...
COSE_Key オブジェクトは、1 つのキー・オブジェクトを保持する方法を定義しています。個々のキー・タイプのメンバを定義する必要があります。このセクションでは、特定のキー・タイプのメンバの初期セットを定義します。
...
それぞれのキー・タイプについて、パブリック・メンバーとプライベート・メンバーの両方を定義する。パブリック・メンバは、他者に公開され、利用されるものである。プライベートメンバは、個人が鍵をアーカイブできるようにするものである。ただし、プロトコルのエンティティに秘密鍵を配布する場合もある。例えば、乱数生成能力の低いエンティティ、マルチキャスト型操作のための 鍵生成の集中化、共有秘密鍵が認可目的のベアラートークンとして使用される プロトコルなどである。

> Key types are identified by the "kty" member of the COSE_Key object. In this document, we define four values for the member:

キー・タイプは、COSE_Key オブジェクトのメンバ "kty "によって識別される。このドキュメントでは、このメンバに 4 つの値を定義します：
...
> Two different key structures are defined for elliptic curve keys. One version uses both an x-coordinate and a y-coordinate, potentially with point compression ("EC2"). This is the conventional elliptic curve (EC) point representation that is used in [[RFC5480](https://www.rfc-editor.org/rfc/rfc9053.html#RFC5480)]. The other version uses only the x-coordinate, as the y-coordinate is either to be recomputed or not needed for the key agreement operation ("OKP").

楕円曲線鍵には2つの異なる鍵構造が定義されている。1つはx座標とy座標の両方を使用するもので、点圧縮("EC2")を使用する可能性が ある。これは[RFC5480]で使用されている従来の楕円曲線(EC)鍵表現である。もう1つのバージョンはx座標のみを使用するもので、y座標は再計算するか、 鍵合意操作に必要ないからである("OKP")。

> Applications MUST check that the curve and the key type are consistent and reject a key if they are not.

アプリケーションは、 curve(crv)とkey type(kty)が一致していることをチェックし、一致していない場合は キーを拒否しなければならない(MUST)。
...
一般的に、プロトコルは楕円曲線点をx座標とy座標、またはx座標とy座標の符号ビットで伝送する。後者のエンコーディングは、潜在的なIPRの問題からIETFでは推奨されていない。しかし、制約のある環境での運用では、y座標を送信しないことで、メッ セージを縮小できる機能は有用である可能性がある。

> For EC keys with both coordinates, the "kty" member is set to 2 (EC2). The key parameters defined in this section are summarized in [Table 19](https://www.rfc-editor.org/rfc/rfc9053.html#x-table-ec2-keys). The members that are defined for this key type are:

両方の座標を持つECキーの場合、"kty "メンバは2（EC2）に設定される。本セクションで定義されるキー・パラメータを表19にまとめる。このキー・タイプに定義されるメンバは以下のとおりである：

> crv: This contains an identifier of the curve to be used with the key. The curves defined in this document for this key type can be found in [Table 18](https://www.rfc-editor.org/rfc/rfc9053.html#x-table-ec-curves). Other curves may be registered in the future, and private curves can be used as well.

crv: これには、そのキーで使用される曲線の識別子が含まれる。本文書で定義されているこの鍵タイプのカーブは、表18に示す。将来、他の曲線が登録されるかもしれないし、プライベート曲線も使用できる。

> x: This contains the x-coordinate for the EC point. The integer is converted to a byte string as defined in [[SEC1](https://www.rfc-editor.org/rfc/rfc9053.html#SEC1)]. Leading-zero octets MUST be preserved.

x: ECポイントのX座標が格納される。整数は[SEC1]で定義されているバイト列に変換される。先頭のゼロオクテットは保存しなければならない(MUST)。


> y: This contains either the sign bit or the value of the y-coordinate for the EC point. When encoding the value y, the integer is converted to a byte string (as defined in [[SEC1](https://www.rfc-editor.org/rfc/rfc9053.html#SEC1)]) and encoded as a CBOR bstr. Leading-zero octets MUST be preserved. Compressed point encoding is also supported. Compute the sign bit as laid out in the Elliptic-Curve-Point-to-Octet-String Conversion function of [[SEC1](https://www.rfc-editor.org/rfc/rfc9053.html#SEC1)]. If the sign bit is zero, then encode y as a CBOR false value; otherwise, encode y as a CBOR true value. The encoding of the infinity point is not supported.

y: これは符号ビットまたはEC点のy座標の値を含む。値 y をエンコードする場合、整数はバイト列（[SEC1]で定義されている）に変換され、 CBOR bstr としてエンコードされる。先頭のゼロオクテットは保存されなければならない(MUST)。圧縮符号化もサポートされている。[SEC1] の Elliptic-Curve-Point-to-Octet-String Conversion 関数で定義されているように符号ビットを計算する。符号ビットが0であれば、yをCBORの偽の値として符号化し、そうでなければ、yをCBORの真の値として符号化する。無限遠点のエンコーディングはサポートされていない。
...
公開鍵の場合、構造体中に「crv」、「x」、「y」が存在することが必須である。秘密鍵の場合、"crv "と "d "が構造体中に存在することが必須である。秘密鍵の場合、"x "と "y "も存在することが推奨される[RECOMMENDED]が、これらは 必要な要素から再計算することができ、省略することで容量を節約できる。
...
- COSE_KEY は JWK とほとんど同じ
  - https://datatracker.ietf.org/doc/html/rfc7517#section-3
- [FIDO2: Conformance testing server API](https://github.com/fido-alliance/conformance-test-tools-resources/blob/fido2-doc-update/docs/FIDO2/Server/Conformance-Test-API.md#fido2-conformance-testing-server-api)
- フロントから送る ClientData.challenge の base64 エンコードされた値に == が入っていない
